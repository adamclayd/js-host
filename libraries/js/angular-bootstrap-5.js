...
(function() {
	...
	tabs.animation('.bs5-tab-pane', ['$animateCss', function($animateCss) {
		return {
	...
	angular.module('angular/bootstrap5/templates/tabs/tabset-top.html', []).run(['$templateCache', function($templateCache) {
		$templateCache.put(
			'angular/bootstrap5/templates/tabs/tabset-top.html',
	...
	angular.module('angular/bootstrap5/templates/tabs/tabset-left.html', []).run(['$templateCache', function($templateCache) {
		$templateCache.put(
			'angular/bootstrap5/templates/tabs/tabset-left.html',
	...
	angular.module('angular/bootstrap5/templates/tabs/tabset-right.html', []).run(['$templateCache', function($templateCache) {
		$templateCache.put(
			'angular/bootstrap5/templates/tabs/tabset-right.html',
	...
	angular.module('angular/bootstrap5/templates/tabs/tabset-bottom.html', []).run(['$templateCache', function($templateCache) {
		$templateCache.put(
			'angular/bootstrap5/templates/tabs/tabset-bottom.html',
	...
	tooltip.directive('bs5Tooltip', ['$templateCache', '$document', '$compile', '$http', '$q', '$bs5Position', '$timeout', '$animateCss', function($templateCache, $document, $compile, $http, $q, $bs5Position, $timeout, $animateCSS) {
		return {
			restrict: 'A',
			link: function(scope, elm, attrs) {
				...
				options.title = attrs.bs5Tooltip;

				let tooltip = new bootstrap.Tooltip(elm[0], options);
					...
					def.promise.then(function(tpl) {
						var Tooltip = function(tooltipEl) {
							...
							this.show = function() {
								...
								let arrowPlacement = placement === 'left' ? 'right' : (placement === 'bottom' ? 'top' : (placement === 'right' ? 'left' : 'bottom'));
								
								$timeout(function() {
									let pos = $bs5Position.positionTarget(elm, el, placement + '-center');


									let regex = /^(top|bottom|left)$/;
									if (regex.test(placement) && pos.left < 0) {
										placement = 'right';
										arrowPlacement = 'left';
										pos = $bs5Position.positionTarget(elm, el, placement + '-center');
									}

									if(placement === 'top' && pos.top < 0) {
										placement = 'bottom';
										arrowPlacement = 'top';
										pos = pos = $bs5Position.positionTarget(elm, el, placement + '-center');
									}

									let off = $bs5Position.offset(el);
									let bodyOff = $bsPosition.offset($document.find('body'));
									regex = /^(top|bottom|right)$/;
									if(regex.test(placement) && pos.left + off.width > bodyOff.width) {
										placement = 'left';
	...
	popover.directive('bs5Popover', ['$templateCache', '$document', '$compile', '$http', '$q', '$timeout', '$bs5Position', '$animateCss', function($templateCache, $document, $compile, $http, $q, $timeout, $bs5Position, $animateCss) {
		...
		return {
			...
			link: function(scope, elm, attrs) {
				...
				let animate = attrs.animate ? scope.$eval(attrs.animate) : true;
				let delay = isNaN(attrs.delay) ? 0 : scope.$eval(attrs.delay);
				let html = attrs.html ? scope.$eval(attrs.html) : false;
				...
				let off = scope.$eval(attrs.offset);
					...
					def.promise.then(function(tpl) {
						...
						let Popover = function(popoverEl) {
							...
							scope.$watch(function() { return elm.attr('title'); },  function($new, $old) {
								if($new !== $old) {
									self.scope.title = elm.attr('title');
								}
							});
							...
							this.show = function() {
								$timeout(function() {
									...
									let arrowPlacement = placement === 'top' ? 'bottom' : (placement === 'left' ? 'right' : (placement === 'bottom' ? 'top' : 'left'));
									$compile(el)(self.scope);


									$timeout(function () {
										let pos = $bs5Position.positionTarget(elm, el, placement + '-center');
										let offset = $bs5Position.offset(el);
										let bodyOff = $bs5Position.offset($document.find('body'));
										...
										regex = /^(top|bottom|right)$/;
										if (regex.test(placement) && pos.left + offset.width > bodyOff.width) {
											placement = 'left';
											arrowPlacement = 'right';
											pos = $bs5Position.positionTarget(elm, el, placement + '-center');
										}
										...
										el.css({
											'position': 'absolute',
											'top': (pos.top + off[1]) + 'px',
											'left': (pos.left + off[0]) + 'px',
											'opacity': animate ? 0 : 1,
											'z-index': 9999
										});

										$timeout(function () {
											let position = $bs5Position.positionTargetRelative(el, arrow, arrowPlacement + '-center');
